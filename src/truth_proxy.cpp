...
//
// Purpose:
// - execute caller-specified blocks of continuation rollouts in parallel;
// - return only compact sufficient-stat increments instead of full rollout
//   paths; and
// - serve as the shared native engine for proxy-truth construction and
//   repeated-seed explicit-posterior study workflows.
//
// Important contrast with alloc_core.cpp:
// - alloc_core.cpp chooses which arm gets the next rollout adaptively;
// - this file assumes the caller already made that decision and only needs the
//   block executed quickly and reproducibly.
//
// Inputs:
// - one board plus legal moves;
// - one requested rollout-block schedule by representative candidate; and
// - one rollout environment (policy, max turns, dice mode, CRN settings).
//
// Outputs:
// - a collapsed-candidate map describing which legal moves share one successor
//   board; and
// - one aggregated sufficient-stat increment row per requested representative.
...
// ---------------------------------------------------------------------------
// Candidate-collapse and rollout-block bookkeeping
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Lightweight rollout execution shared by truth building and study blocks
// ---------------------------------------------------------------------------
...
Rcpp::List bg_cpp_rollout_blocks(
  ...
  if (candidate_index.size() == 0) {
    // Return the collapsed candidate map even when no new rollouts were
    // requested. This keeps the caller's bookkeeping deterministic.
  ...
  if (normalized_start_counts.size() == 0) {
    // When the caller omits running totals, every requested block is treated as
    // starting from sample index 1 for its candidate.
