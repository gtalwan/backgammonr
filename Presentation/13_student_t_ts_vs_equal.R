# 13_student_t_ts_vs_equal.R
#
# Purpose:
# - compare baseline Thompson sampling against equal allocation under the
#   scalar-payoff + Student-t stack;
# - use the preserved master truths projected into the Student-t model; and
...
study <- bg_opening_compare_study(
  ...
  save_path = file.path(
    ...
    "13_student_t_ts_vs_equal_master_2048.rds"
  ),
  overwrite = TRUE,
  seed = 1L
)
...
# This is the headline performance curve for the Student-t stack.
plot_top1 <- ggplot(
  ...
  labs(
    title = "Student-t stack: TS versus equal",
    subtitle = paste(
      "Both methods are evaluated against the same projected master truths at reference budget",
...
plot_regret <- ggplot(
  ...
  scale_x_continuous(trans = "log2", breaks = budgets) +
  labs(
    title = "Student-t stack: simple regret",
...
plot_focus <- ggplot(
  ...
  scale_x_continuous(trans = "log2", breaks = budgets) +
  labs(
    title = "Student-t stack: budget share on the truth top-2",
...
plot_conf_wrong <- ggplot(
  ...
  scale_x_continuous(trans = "log2", breaks = budgets) +
  labs(
    title = "Student-t stack: high-confidence wrong rate",
    subtitle = "Lower is better.",
    x = "Budget",
...
# This heatmap shows exactly which openings drive the Student-t head-to-head.
heatmap_plot <- ggplot(
  opening_contrast,
  aes(x = "Student-t", y = factor(opening_roll, levels = unique(truths$summary$opening_roll)), fill = ts_minus_equal_top1)
) +
  geom_tile(color = "white", linewidth = 0.2) +
  scale_fill_gradient2(low = "#0072B2", mid = "#F4F1EA", high = "#D55E00", midpoint = 0) +
  labs(
    title = "Student-t stack: TS minus equal by opening",
...
hardness_plot <- ggplot(
  ...
  labs(
    title = "Student-t stack: where TS gains appear",
...
presentation_save_table(final_table, repo_root, "13_student_t_ts_vs_equal_final_table")
presentation_save_table(opening_contrast, repo_root, "13_student_t_ts_vs_equal_opening_contrast")
presentation_save_plot(plot_top1, repo_root, "13_student_t_ts_vs_equal_top1", width = 10, height = 6)
presentation_save_plot(plot_regret, repo_root, "13_student_t_ts_vs_equal_regret", width = 10, height = 6)
presentation_save_plot(plot_focus, repo_root, "13_student_t_ts_vs_equal_focus", width = 10, height = 6)
presentation_save_plot(plot_conf_wrong, repo_root, "13_student_t_ts_vs_equal_conf_wrong", width = 10, height = 6)
presentation_save_plot(heatmap_plot, repo_root, "13_student_t_ts_vs_equal_heatmap", width = 8, height = 8)
presentation_save_plot(hardness_plot, repo_root, "13_student_t_ts_vs_equal_hardness", width = 10, height = 6)
